...
Install commands are copied verbatim from [.agents/install-block.md](./.agents/install-block.md). `.claude-plugin/marketplace.json` makes this fork its own single-plugin marketplace for direct testing; it is not the documented installation route. Run `claude plugin validate . --strict` after touching either manifest. The inherited rationale for a Claude plugin but not a Codex one lives in [.agents/adr/0002-ship-as-a-claude-code-plugin.md](./.agents/adr/0002-ship-as-a-claude-code-plugin.md).
