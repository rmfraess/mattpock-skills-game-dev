...
`triage` works through incoming issues/PRs, verifies each claim against its material project surfaces, recommends a category/state, and leaves an honestly routed brief, a specific request for information, or a durable closing reason.

It is for work that arrived without your planning flow: raw bug reports, incoming feature requests, or external pull requests. [to-tickets](https://aihero.dev/skills-to-tickets) already classifies originated tickets by owner, gate, and evidence, so triaging them again is wasted work and can erase specialist/HITL/blocked states.
...
Those five states are the portable minimum. Triage also records a **gate** (unblocked, blocked, deferred, awaiting verification), **route/owner** (autonomous, named specialist, human/creative HITL, maintainer), and evidence authority. `ready-for-agent` applies only when unblocked, safely delegable, and free of remaining human/creative/specialist gates. If the tracker cannot express an honest state, the item remains in triage with the gap documented.
...
Before [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling), `triage` runs the smallest faithful evidence protocol. It preserves dirty/generated/editor state before PR checkout; verifies mixed source/content PRs separately; reproduces visual/content issues in the named scene; records hardware/build/map/settings/path/capture for performance; and checks network topology, package, or target conditions when material. Source/lore conflicts route to approved source/adaptation authority rather than code-only implementation.
...
All of it exists to make one artifact good: the **owner brief** posted when work becomes honestly actionable. It names behavior, route/gate, falsifiable evidence, owner/reviewer, and stable interfaces. Incidental source paths/line numbers are omitted, but an exact shared map/scene/package/asset identity is included when locking, reproduction, or evidence depends on it.
...
**The agent tried to apply a state and the tracker label doesn't exist.**
Run [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) verification. It checks that mapped labels/roles and wayfinding operations exist, reports missing prerequisites, and asks before creating or changing remote configuration. Do not fall back to a misleading existing label.

**Five states aren't enough—what about blocked, deferred, specialist, creative, or awaiting verification?**
Use the project's richer state mapping when available and always include gate/route/evidence fields in the brief. Without a matching tracker state, keep the item in triage and document the missing prerequisite instead of calling it agent-ready.

**How is this different from `/diagnosing-bugs`?**
Triage asks whether the claim is real and what evidence/owner is needed; [diagnosing-bugs](https://aihero.dev/skills-diagnosing-bugs) finds the root cause through a controlled repro protocol. Route there when verification establishes a real hard bug but cannot explain it.
...
- The issue/PR body and every relevant comment were read before any state change, including in bulk passes.
- The claim was checked through every material source/editor/runtime/evidence surface before readiness.
- Briefs name route, gate, owner/reviewer, falsifiable evidence, and exact shared artifact identity only where safety needs it.
- Blocked, deferred, specialist, creative/HITL, autonomous, and awaiting-verification work do not collapse into one ready state.
